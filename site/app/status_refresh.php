...
function drama_status_refresh(PDO $pdo, int $pageId, string $asOf = '', bool $save = true): array {
    ...
    $nxTxt = '';
    foreach ($f['whats_next'] as $nx) $nxTxt .= '- ' . ($nx['date'] !== '' ? "[{$nx['date']}] " : '') . $nx['text'] . "\n";
    ...
    $sys = "You are GenZHype's status desk. New dated events reached this story's timeline after its summary was written, so the summary, status and FAQ may now be out of date or contradict the timeline. Update ONLY what the timeline makes out of date.\n"
         ...
         . "6. whats_next: the page's WHAT HAPPENS NEXT list brought up to the timeline: drop items the timeline shows already happened or were called off, keep the rest, add what the latest events say is scheduled or awaited. At most 3, each with its date when a source gives one, each attributed; [] when nothing is pending.\n"
         . "7. changed: false when the current summary already reflects the latest events and the right status; then return the current values.\n"
         . "Output STRICT JSON only: {\"summary\":\"...\",\"lifecycle\":\"ongoing|resolved|dormant\",\"meta_desc\":\"...\",\"status_faq\":{\"q\":\"...\",\"a\":\"...\"},\"whats_next\":[{\"date\":\"same format as timeline dates, or empty\",\"text\":\"...\"}],\"changed\":true,\"why\":\"one short sentence\"}";
    ...
    $hosts = fact_hosts(array_merge(array_column($f['events'], 'host'), array_column($f['events'], 'who')));
    ...
    for ($try = 0; ; $try++) {
        ...
        if (!is_array($j) || !isset($j['summary'])) {
            ...
            // 'developing' is the gate's label for a thin ongoing story (gate.php, option A); keep it
            if ($life === 'ongoing' && $f['lifecycle'] === 'developing') $life = 'developing';
    ...
    // what happens next: checked like the drafter's (story_context_clean); a reply without the key keeps the list
    $next = $f['whats_next']; $nextDropped = [];
    if (isset($j['whats_next']) && is_array($j['whats_next'])) {
        $nc = story_context_clean('', $j['whats_next'], $corpus, $hosts);
        $next = $nc['next']; $nextDropped = $nc['dropped'];
    }
    $nextChanged = json_encode($next) !== json_encode($f['whats_next']);
    $out = ['ok' => true, 'changed' => true, 'why' => (string)($j['why'] ?? ''), 'summary' => $summary, 'lifecycle' => $life,
            'meta_desc' => $meta, 'status_faq' => ['q' => $sq, 'a' => $sa], 'whats_next' => $next, 'next_dropped' => $nextDropped];
    ...
    try {
        ...
        if ($nextChanged) {
            $hist->execute([$pageId, 'whats_next', json_encode($f['whats_next'], JSON_UNESCAPED_UNICODE), json_encode($next, JSON_UNESCAPED_UNICODE)]);
            $pdo->prepare("UPDATE dramas SET whats_next=? WHERE id=?")->execute([$next ? json_encode($next, JSON_UNESCAPED_UNICODE) : null, $f['drama_id']]);
        }
        if ($sq !== '' && $sa !== '') {
