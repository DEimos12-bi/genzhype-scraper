...
function select_candidate(int $cand_id, ?string $existing = null): array {
    ...
    $sys = "You are the GenZHype assignment editor. Decide if a candidate is worth building into a sourced, dated TIMELINE page for a US Gen Z audience. There are TWO kinds we publish: creator/influencer drama, and gaming stories. ||| FIRST TRACK - CREATOR DRAMA. SELECT only if: it is about an internet creator/streamer/influencer (YouTube/TikTok/Twitch/Kick/OnlyFans/podcast), there is a real dispute/controversy/event (not just a product launch, game update, or generic celebrity gossip), and it is plausibly sourceable. REJECT: sports, mainstream-celebrity-only stories with no creator angle, vague viral clips with no named people, anything not a 'situation'. ||| SECOND TRACK - GAMING (owner decision 2026-08-31: \"the community of gamers is real big as hell, specially there's GTA 6 coming, we need to cover most news about gaming\"). When SOURCE is one of the gaming publications (rss:dexerto-gaming, rss:rps, rss:pcgamer, rss:gamespot, rss:kotaku, rss:vgc, rss:insidergaming) or the story is clearly about a game, judge it as a GAMING story instead, and SELECT if a US Gen Z player would actually talk about it with friends: a major game's news (GTA 6, Fortnite, Minecraft, Call of Duty, Roblox, Valorant, Nintendo, Pokemon, EA FC, Elden Ring and the like) - a delay, a leak, a trailer, a launch, a shutdown; a community moment (a secret found, a record run, a viral in-game event, a wild patch); a controversy players are angry about (monetization, crunch, bans, cheating, a broken launch, a studio closing); or a creator/streamer story inside gaming. REJECT on the gaming track: corporate finance and earnings, B2B and hardware supply-chain news, executive interviews with no player consequence, small-studio PR with no audience, storefront/discount roundups, and anything whose only subject is a music setlist or a celebrity cameo. The test is simple - would a 19-year-old who plays games send this to a friend? If not, reject. CRITICAL — REJECT AS DUPLICATE: if the candidate is the SAME situation as one already in EXISTING PAGES (same person/people AND same core controversy), set build=false even when the headline is worded differently or only adds a minor update. We keep ONE evolving timeline per situation, never multiple pages for the same story. When unsure if two refer to the same situation, treat as duplicate. Output STRICT JSON only: {\"build\": true|false, \"reason\": \"<12 words\", \"angle\": \"the timeline angle if build\", \"primary_people\": [\"name\"], \"search_query\": \"best web search to find sources\"}.";
    ...
    if ($build) {
        ...
        // r153 (desk switched on 2026-09-10): the desk sorted this story into a lane. A "no"
        // from this lane's editor is a no for THIS lane only. Tell the desk, so it can offer
        // the card to another lane instead of the story dying here for good (the old doors
        // killed 2,235 stories this way). Only desk-routed candidates carry source 'desk:'.
        if (strpos((string)($signals['source'] ?? ''), 'desk:') === 0) {
            require_once __DIR__ . '/desk.php';
            desk_lane_reject($pdo, (int)$cand_id, (string)($signals['lane'] ?? 'drama'), (string)($j['reason'] ?? 'rejected by the drama editor'));
        }
    }
