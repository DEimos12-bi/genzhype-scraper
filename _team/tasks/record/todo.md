...
- Verify: [ ] lint [ ] post-deploy: `php app/cli.php record-selftest` → PASS (SSH session)
...
- Verify: [ ] lint [ ] `php app/cli.php record <page_id>` shows build
...
- Verify: [ ] lint [ ] `record 700` shows plan
...
- Verify: [ ] lint [ ] after next drop ingest, `record <pid>` shows it
...
- Verify: [ ] lint [ ] `record <pid>` shows outcome after a collection run
...
- Verify: [ ] lint [ ] curl from local with token POSTed (read from local config, never printed)
...
- Verify: [ ] lint [ ] end-to-end click test
